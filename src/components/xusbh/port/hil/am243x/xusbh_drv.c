...
// @file xusbh_drv.c
// @brief AM243x USB Host Controller Driver port scaffold.
...
// SYSTEM INCLUDES
#if defined(xUSBH_AM243X_HCD_ENABLE_MCU_PLUS_SDK)
...
#if defined(xUSBH_AM243X_HCD_ENABLE_SUPER_SPEED)
...
// MACROS /////////////////////////////////////////////////////////////////////////
#define xUSBH_AM243X_HCD_USBSSP_ROOT_PORT                1U
#define xUSBH_AM243X_HCD_MODESTRAP_HOST                  1U
#define xUSBH_AM243X_HCD_READY_TIMEOUT_CYCLES            1000000U
#define xUSBH_AM243X_HCD_USB3_REFCLK_HZ                  100000000U
#define xUSBH_AM243X_HCD_USB3_LANE_FUNC_USB              1U
#define xUSBH_AM243X_HCD_SERDES0_CLKSEL_MAIN_PLL2_HSDIV4 3U
#define xUSBH_AM243X_HCD_ENDPOINT_DESC_SIZE              7U
#define xUSBH_AM243X_HCD_ENDPOINT_ADDR_EP0               0U

#if defined(xUSBH_AM243X_HCD_ENABLE_MCU_PLUS_SDK)
#define xUSBH_AM243X_HCD_WRITE_REG_FIELD32(reg_addr, reg_field, value)                                                                      \
    am243x_hcd_write_field32((uintptr_t)(reg_addr), (uint32_t)reg_field##_MASK, (uint32_t)reg_field##_SHIFT, (uint32_t)(value))
#define xUSBH_AM243X_HCD_READ_REG_FIELD32(reg_addr, reg_field)                                                                              \
    am243x_hcd_read_field32((uintptr_t)(reg_addr), (uint32_t)reg_field##_MASK, (uint32_t)reg_field##_SHIFT)
#endif

// TYPES //////////////////////////////////////////////////////////////////////////
#if defined(xUSBH_AM243X_HCD_ENABLE_MCU_PLUS_SDK)
typedef struct am243x_xhci_memory_t
{
...
} am243x_xhci_memory_t;

typedef struct am243x_hcd_endpoint_state_t
{
    xUSBH_Transfer_t *transfer;
    uint8_t endpoint_address;
    bool is_enabled;
    bool is_configuring;
} am243x_hcd_endpoint_state_t;
#endif

// VARIABLES //////////////////////////////////////////////////////////////////////
xUSBH_AM243x_HCD_Context_t xUSBH_AM243x_HCD_Context = {0};
...
static am243x_xhci_memory_t xhci_memory __attribute__((section(".bss.nocache")));
static am243x_hcd_endpoint_state_t endpoint_states[USBSSP_EP_CONT_MAX];
#endif

// FUNCTION PROTOTYPES /////////////////////////////////////////////////////////////
static bool am243x_hcd_ctx_is_valid(const xUSBH_AM243x_HCD_Context_t *ctx);
#if defined(xUSBH_AM243X_HCD_ENABLE_MCU_PLUS_SDK)
static xRETURN_t am243x_hcd_notify_port_event(xUSBH_AM243x_HCD_Context_t *ctx, xUSBH_HCD_Port_Event_t event);
static xRETURN_t
am243x_hcd_notify_transfer_event(xUSBH_AM243x_HCD_Context_t *ctx, xUSBH_Transfer_t *transfer, xUSBH_HCD_Transfer_Event_t event);
#endif
static xRETURN_t am243x_hcd_init(void *hcd_ctx, void *host_ctx, xUSBH_HCD_Event_Callback_t callback);
static xRETURN_t am243x_hcd_deinit(void *hcd_ctx);
static xRETURN_t am243x_hcd_start(void *hcd_ctx);
static xRETURN_t am243x_hcd_stop(void *hcd_ctx);
static xRETURN_t am243x_hcd_enable_interrupts(void *hcd_ctx);
static xRETURN_t am243x_hcd_disable_interrupts(void *hcd_ctx);
static xRETURN_t am243x_hcd_port_power(void *hcd_ctx, uint8_t port, bool enable);
static xRETURN_t am243x_hcd_port_reset(void *hcd_ctx, uint8_t port);
static xRETURN_t am243x_hcd_get_port_status(void *hcd_ctx, uint8_t port, xUSBH_HCD_Port_Status_t *status);
static xRETURN_t am243x_hcd_submit_transfer(void *hcd_ctx, xUSBH_Transfer_t *transfer);
static xRETURN_t am243x_hcd_cancel_transfer(void *hcd_ctx, xUSBH_Transfer_t *transfer);
static uint32_t am243x_hcd_get_frame_number(void *hcd_ctx);
#if defined(xUSBH_AM243X_HCD_ENABLE_MCU_PLUS_SDK)
static uint32_t am243x_hcd_read32(uintptr_t address);
static void am243x_hcd_write32(uintptr_t address, uint32_t value);
static uint32_t am243x_hcd_read_field32(uintptr_t address, uint32_t mask, uint32_t shift);
static void am243x_hcd_write_field32(uintptr_t address, uint32_t mask, uint32_t shift, uint32_t value);
static xRETURN_t am243x_hcd_status_from_cdn(uint32_t status);
static USB_Speed_t am243x_hcd_speed_from_ch9(CH9_UsbSpeed speed);
static void am243x_hcd_configure_xhci_memory(void);
static bool am243x_hcd_is_super_speed_enabled(void);
static uint8_t am243x_hcd_endpoint_index_from_address(uint8_t endpoint_address);
static xRETURN_t am243x_hcd_transfer_endpoint_index(xUSBH_AM243x_HCD_Context_t *ctx, xUSBH_Transfer_t *transfer, uint8_t *ep_index);
static xRETURN_t am243x_hcd_find_device(const xUSBH_Context_t *host_ctx,
                                       uint8_t device_address,
                                       const xUSBH_Device_Context_t **device_ctx,
                                       uint8_t *device_index);
static xRETURN_t am243x_hcd_find_endpoint(const xUSBH_Context_t *host_ctx,
                                         uint8_t device_address,
                                         uint8_t endpoint_address,
                                         const xUSBH_Endpoint_Context_t **endpoint_ctx);
static void am243x_hcd_ch9_setup_from_transfer(const xUSBH_Transfer_t *transfer, CH9_UsbSetup *setup);
static void am243x_hcd_endpoint_descriptor_build(const xUSBH_Endpoint_Context_t *endpoint_ctx,
                                                uint8_t descriptor[xUSBH_AM243X_HCD_ENDPOINT_DESC_SIZE]);
static xRETURN_t am243x_hcd_cache_before_submit(const xUSBH_Transfer_t *transfer);
static void am243x_hcd_cache_after_complete(const xUSBH_Transfer_t *transfer);
static xUSBH_HCD_Transfer_Event_t am243x_hcd_transfer_event_from_cdn(uint32_t status, const USBSSP_RingElementT *event_ptr);
static void am243x_hcd_transfer_actual_length_set(xUSBH_Transfer_t *transfer, const USBSSP_RingElementT *event_ptr);
static void am243x_hcd_transfer_complete_common(USBSSP_DriverResourcesT *resources,
                                               uint32_t status,
                                               const USBSSP_RingElementT *event_ptr,
                                               uint8_t ep_index);
static void am243x_hcd_control_complete(USBSSP_DriverResourcesT *resources, uint32_t status, const USBSSP_RingElementT *event_ptr);
static void am243x_hcd_data_complete(USBSSP_DriverResourcesT *resources, uint32_t status, const USBSSP_RingElementT *event_ptr);
static xRETURN_t am243x_hcd_config_descriptor_seed(xUSBH_AM243x_HCD_Context_t *ctx, const xUSBH_Transfer_t *transfer);
static xRETURN_t am243x_hcd_enable_configured_endpoints(xUSBH_AM243x_HCD_Context_t *ctx, const xUSBH_Transfer_t *transfer);
static void am243x_hcd_issue_configure_endpoint_command(void);
static void am243x_hcd_configure_complete_poll(xUSBH_AM243x_HCD_Context_t *ctx);
#if defined(xUSBH_AM243X_HCD_ENABLE_SUPER_SPEED)
static xRETURN_t am243x_hcd_enable_usb3_phy(void);
#endif
static xRETURN_t am243x_hcd_wait_for_host_ready(void);
static xRETURN_t am243x_hcd_enable_usb0_host(void);
static xRETURN_t am243x_hcd_init_usbssp(xUSBH_AM243x_HCD_Context_t *ctx);
static xRETURN_t am243x_hcd_poll_port_connection(xUSBH_AM243x_HCD_Context_t *ctx, bool notify);
#endif

// MODULE FUNCTIONS IMPLEMENTATION ////////////////////////////////////////////////
static bool am243x_hcd_ctx_is_valid(const xUSBH_AM243x_HCD_Context_t *ctx)
{
    return (ctx != NULL) && (ctx->port == xUSBH_AM243X_HCD_USB0_PORT);
}
...
static uint32_t am243x_hcd_read_field32(uintptr_t address, uint32_t mask, uint32_t shift)
{
    return (am243x_hcd_read32(address) & mask) >> shift;
}

static void am243x_hcd_write_field32(uintptr_t address, uint32_t mask, uint32_t shift, uint32_t value)
{
    uint32_t reg_value = am243x_hcd_read32(address);

    reg_value &= ~mask;
    reg_value |= (value << shift) & mask;
    am243x_hcd_write32(address, reg_value);
}
                                         ...
                                         const xUSBH_Endpoint_Context_t **endpoint_ctx)
{
    ...
    ret = am243x_hcd_find_device(host_ctx, device_address, &device_ctx, &device_index);
    if (ret != xRETURN_OK)
    {
        return ret;
    }
...
static xRETURN_t am243x_hcd_transfer_endpoint_index(xUSBH_AM243x_HCD_Context_t *ctx, xUSBH_Transfer_t *transfer, uint8_t *ep_index)
{
    ...
    if ((transfer->endpoint_address == xUSBH_AM243X_HCD_ENDPOINT_ADDR_EP0) || (transfer->endpoint_type == USB_ENDP_TYPE_CTRL))
    {
        *ep_index = USBSSP_EP0_CONT_OFFSET;
        return xRETURN_OK;
    }

    if (am243x_hcd_find_endpoint((const xUSBH_Context_t *)ctx->host_ctx, transfer->device_address, transfer->endpoint_address,
                                &endpoint_ctx) != xRETURN_OK)
    {
        return xRETURN_xERR_xUSBH_INVALID_OBJECT;
    }

    index = am243x_hcd_endpoint_index_from_address(endpoint_ctx->endpoint_address);
...
static void am243x_hcd_transfer_complete_common(USBSSP_DriverResourcesT *resources,
                                               ...
                                               uint8_t ep_index)
{
    xUSBH_AM243x_HCD_Context_t *ctx = &xUSBH_AM243x_HCD_Context;
    xUSBH_Transfer_t *transfer;
    ...
    event = am243x_hcd_transfer_event_from_cdn(status, event_ptr);
    am243x_hcd_transfer_actual_length_set(transfer, event_ptr);
    am243x_hcd_cache_after_complete(transfer);

    if ((endpoint_states[ep_index].is_configuring == true) && (event == xUSBH_HCD_TRANSFER_EVENT_COMPLETE))
    {
        if (am243x_hcd_enable_configured_endpoints(ctx, transfer) == xRETURN_OK)
        {
    ...
    endpoint_states[ep_index].is_configuring = false;
    (void)am243x_hcd_notify_transfer_event(ctx, transfer, event);
}

static void am243x_hcd_control_complete(USBSSP_DriverResourcesT *resources, uint32_t status, const USBSSP_RingElementT *event_ptr)
{
    am243x_hcd_transfer_complete_common(resources, status, event_ptr, USBSSP_EP0_CONT_OFFSET);
}
...
static void am243x_hcd_data_complete(USBSSP_DriverResourcesT *resources, uint32_t status, const USBSSP_RingElementT *event_ptr)
{
    ...
    am243x_hcd_transfer_complete_common(resources, status, event_ptr, ep_index);
}
...
static xRETURN_t am243x_hcd_enable_configured_endpoints(xUSBH_AM243x_HCD_Context_t *ctx, const xUSBH_Transfer_t *transfer)
{
    ...
    if (am243x_hcd_find_device(host_ctx, transfer->device_address, &device_ctx, &device_index) != xRETURN_OK)
    {
        return xRETURN_xERR_xUSBH_INVALID_OBJECT;
    }
    ...
    for (uint8_t i = 0U; i < xUSBH_MAX_ENDPOINTS; i++)
    {
        if ((host_ctx->endpoints[i].is_allocated == true) && (host_ctx->endpoints[i].device_index == device_index))
        {
            uint8_t descriptor[xUSBH_AM243X_HCD_ENDPOINT_DESC_SIZE] = {0};
            uint8_t ep_index = am243x_hcd_endpoint_index_from_address(host_ctx->endpoints[i].endpoint_address);
            uint32_t cdn_status;
            ...
            am243x_hcd_endpoint_descriptor_build(&host_ctx->endpoints[i], descriptor);
            cdn_status = p_usbssp_obj->enableEndpoint(&usbssp_resources, descriptor);
            if (cdn_status != CDN_EOK)
            {
                return am243x_hcd_status_from_cdn(cdn_status);
            }
    ...
    am243x_hcd_issue_configure_endpoint_command();
    return xRETURN_OK;
}
...
static void am243x_hcd_configure_complete_poll(xUSBH_AM243x_HCD_Context_t *ctx)
{
    ...
    {
        (void)am243x_hcd_notify_transfer_event(ctx, transfer, xUSBH_HCD_TRANSFER_EVENT_COMPLETE);
    }
    else
    {
        (void)am243x_hcd_notify_transfer_event(ctx, transfer, xUSBH_HCD_TRANSFER_EVENT_ERROR);
    }
}
...
static xRETURN_t am243x_hcd_enable_usb3_phy(void)
{
    ...
    xUSBH_AM243X_HCD_WRITE_REG_FIELD32((CSL_CTRL_MMR0_CFG0_BASE + CSL_MAIN_CTRL_MMR_CFG0_SERDES0_LN0_CTRL),
                                      CSL_MAIN_CTRL_MMR_CFG0_SERDES0_LN0_CTRL_LANE_FUNC_SEL, xUSBH_AM243X_HCD_USB3_LANE_FUNC_USB);
    xUSBH_AM243X_HCD_WRITE_REG_FIELD32((CSL_CTRL_MMR0_CFG0_BASE + CSL_MAIN_CTRL_MMR_CFG0_SERDES0_CLKSEL),
                                      CSL_MAIN_CTRL_MMR_CFG0_SERDES0_CLKSEL_CORE_REFCLK_SEL,
                                      xUSBH_AM243X_HCD_SERDES0_CLKSEL_MAIN_PLL2_HSDIV4);

    if (am243x_wiz_init(CSL_SERDES_10G0_BASE, PHY_TYPE_USB3, 0U, xUSBH_AM243X_HCD_USB3_REFCLK_HZ) != 0)
    {
        return xRETURN_xERR_xUSBH_INVALID_STATE;
    }
...
static xRETURN_t am243x_hcd_wait_for_host_ready(void)
{
    uint32_t timeout = xUSBH_AM243X_HCD_READY_TIMEOUT_CYCLES;

    while ((xUSBH_AM243X_HCD_READ_REG_FIELD32(&p_usbregs_ctlr->DRD.OTGSTS, CSL_USB3P0SS_CTRL_DRD_OTGSTS_OTG_NRDY) != 0U) && (timeout > 0U))
    {
        timeout--;
    }
    if (timeout == 0U)
    ...
    timeout = xUSBH_AM243X_HCD_READY_TIMEOUT_CYCLES;
    while ((xUSBH_AM243X_HCD_READ_REG_FIELD32(&p_usbregs_ctlr->DRD.OTGSTS, CSL_USB3P0SS_CTRL_DRD_OTGSTS_XHC_READY) == 0U) && (timeout > 0U))
    {
        timeout--;
    }
    if (timeout == 0U)
...
static xRETURN_t am243x_hcd_enable_usb0_host(void)
{
...
#if defined(xUSBH_AM243X_HCD_ENABLE_SUPER_SPEED)
    ret = am243x_hcd_enable_usb3_phy();
    if (ret != xRETURN_OK)
    {
    ...
    xUSBH_AM243X_HCD_WRITE_REG_FIELD32(&p_usbregs_tiwrap->USB3P0SS_W1, CSL_USB3P0SS_CMN_USB3P0SS_W1_PWRUP_RST_N, 0U);
    xUSBH_AM243X_HCD_WRITE_REG_FIELD32(&p_usbregs_tiwrap->USB3P0SS_W1, CSL_USB3P0SS_CMN_USB3P0SS_W1_MODESTRAP_SEL, 1U);
    xUSBH_AM243X_HCD_WRITE_REG_FIELD32(&p_usbregs_tiwrap->USB3P0SS_W1, CSL_USB3P0SS_CMN_USB3P0SS_W1_MODESTRAP,
                                      xUSBH_AM243X_HCD_MODESTRAP_HOST);
    xUSBH_AM243X_HCD_WRITE_REG_FIELD32(&p_usbregs_tiwrap->USB3P0SS_W1, CSL_USB3P0SS_CMN_USB3P0SS_W1_USB2_ONLY_MODE,
                                      (am243x_hcd_is_super_speed_enabled() == true) ? 0U : 1U);
    xUSBH_AM243X_HCD_WRITE_REG_FIELD32(&p_usbregs_tiwrap->STATIC_CONFIG, CSL_USB3P0SS_CMN_STATIC_CONFIG_VBUS_SEL, 1U);
    xUSBH_AM243X_HCD_WRITE_REG_FIELD32(&p_usbregs_tiwrap->STATIC_CONFIG, CSL_USB3P0SS_CMN_STATIC_CONFIG_PLL_REF_SEL, 0x06U);
    xUSBH_AM243X_HCD_WRITE_REG_FIELD32(&p_usbregs_tiwrap->USB3P0SS_W1, CSL_USB3P0SS_CMN_USB3P0SS_W1_PWRUP_RST_N, 1U);
    am243x_hcd_write32((uintptr_t)&p_usbregs_ctlr->DRD.SESSVALID_DBNC_CFG, 0x000A0002U);

    ret = am243x_hcd_wait_for_host_ready();
...
static xRETURN_t am243x_hcd_init_usbssp(xUSBH_AM243x_HCD_Context_t *ctx)
{
    ...
    am243x_hcd_configure_xhci_memory();
    usbssp_resources.xhciMemRes = &usbssp_mem_resources;
    usbssp_resources.actualPort = xUSBH_AM243X_HCD_USBSSP_ROOT_PORT;
    usbssp_resources.actualSpeed = CH9_USB_SPEED_UNKNOWN;
    usbssp_resources.deviceModeFlag = 0U;
    usbssp_resources.usbModeFlag = (am243x_hcd_is_super_speed_enabled() == true) ? 3U : 2U;
    usbssp_resources.instanceNo = xUSBH_AM243X_HCD_USB0_INSTANCE;
...
static xRETURN_t am243x_hcd_poll_port_connection(xUSBH_AM243x_HCD_Context_t *ctx, bool notify)
{
    ...
    ret = am243x_hcd_status_from_cdn(cdn_status);
    if (ret != xRETURN_OK)
    {
        return ret;
    }
    ...
    ctx->speed = am243x_hcd_speed_from_ch9(usbssp_resources.actualSpeed);

    if ((notify == true) && (ctx->is_port_connected != was_connected))
    {
        if (ctx->is_port_connected == true)
        {
            ret = am243x_hcd_notify_port_event(ctx, xUSBH_HCD_PORT_EVENT_CONNECTED);
        }
        else
        {
            ret = am243x_hcd_notify_port_event(ctx, xUSBH_HCD_PORT_EVENT_DISCONNECTED);
        }
    }
...
static xRETURN_t am243x_hcd_init(void *hcd_ctx, void *host_ctx, xUSBH_HCD_Event_Callback_t callback)
{
    xUSBH_AM243x_HCD_Context_t *ctx = (xUSBH_AM243x_HCD_Context_t *)hcd_ctx;
    ...
    ctx->port = xUSBH_AM243X_HCD_USB0_PORT;
...
static xRETURN_t am243x_hcd_start(void *hcd_ctx)
{
    ...
    if (am243x_hcd_ctx_is_valid(ctx) == false)
    {
        return xRETURN_xERR_xUSBH_NULL_POINTER;
    }
    ...
    {
        xRETURN_t ret = am243x_hcd_enable_usb0_host();
        if (ret != xRETURN_OK)
        {
            return ret;
        }

        ret = am243x_hcd_init_usbssp(ctx);
        if (ret != xRETURN_OK)
        {
            return ret;
        }
    ...
    (void)am243x_hcd_poll_port_connection(ctx, true);
...
static xRETURN_t am243x_hcd_port_power(void *hcd_ctx, uint8_t port, bool enable)
{
    ...
    if (port != xUSBH_AM243X_HCD_USB0_PORT)
    {
        return xRETURN_xERR_xUSBH_INVALID_ARGUMENT;
    }
...
static xRETURN_t am243x_hcd_port_reset(void *hcd_ctx, uint8_t port)
{
    ...
    xRETURN_t ret = am243x_hcd_status_from_cdn(p_usbssp_obj->resetRootHubPort(&usbssp_resources));
    if (ret == xRETURN_OK)
    {
        ret = am243x_hcd_notify_port_event(ctx, xUSBH_HCD_PORT_EVENT_RESET_COMPLETE);
    }
...
static xRETURN_t am243x_hcd_get_port_status(void *hcd_ctx, uint8_t port, xUSBH_HCD_Port_Status_t *status)
{
    ...
    if ((am243x_hcd_ctx_is_valid(ctx) == false) || (status == NULL))
    {
        return xRETURN_xERR_xUSBH_NULL_POINTER;
    }
    if (port != xUSBH_AM243X_HCD_USB0_PORT)
    ...
    xRETURN_t ret = am243x_hcd_poll_port_connection(ctx, false);
    if (ret != xRETURN_OK)
    {
        return ret;
    }
...
static xRETURN_t am243x_hcd_submit_transfer(void *hcd_ctx, xUSBH_Transfer_t *transfer)
{
    ...
    if ((am243x_hcd_ctx_is_valid(ctx) == false) || (transfer == NULL))
    {
        return xRETURN_xERR_xUSBH_NULL_POINTER;
    }
    ...
    ret = am243x_hcd_transfer_endpoint_index(ctx, transfer, &ep_index);
    if (ret != xRETURN_OK)
    {
        return ret;
    }
    ...
    ret = am243x_hcd_cache_before_submit(transfer);
    if (ret != xRETURN_OK)
    {
        return ret;
    }
    ...
    if ((transfer->endpoint_type == USB_ENDP_TYPE_CTRL) || (transfer->endpoint_address == xUSBH_AM243X_HCD_ENDPOINT_ADDR_EP0))
    {
        CH9_UsbSetup setup = {0};
        uint32_t cdn_status;

        am243x_hcd_ch9_setup_from_transfer(transfer, &setup);
        if ((setup.bRequest == USB_REQ_SET_ADDRESS) && ((setup.bmRequestType & USB_REQ_TYPE_MASK) == USB_REQ_TYPE_STANDARD))
        {
            cdn_status = p_usbssp_obj->noOpTest(&usbssp_resources, am243x_hcd_control_complete);
            if (cdn_status != CDN_EOK)
            {
                endpoint_states[ep_index].transfer = NULL;
                return am243x_hcd_status_from_cdn(cdn_status);
        ...
        {
            ret = am243x_hcd_config_descriptor_seed(ctx, transfer);
            if (ret != xRETURN_OK)
            {
                endpoint_states[ep_index].transfer = NULL;
                return ret;
        ...
        cdn_status = p_usbssp_obj->nBControlTransfer(&usbssp_resources, &setup, transfer->data, am243x_hcd_control_complete);
        if (cdn_status != CDN_EOK)
        {
            endpoint_states[ep_index].transfer = NULL;
            endpoint_states[ep_index].is_configuring = false;
    ...
    uint32_t cdn_status =
        p_usbssp_obj->transferData(&usbssp_resources, ep_index, (uintptr_t)transfer->data, transfer->length, am243x_hcd_data_complete);
    if (cdn_status != CDN_EOK)
    {
        endpoint_states[ep_index].transfer = NULL;
        return am243x_hcd_status_from_cdn(cdn_status);
...
static xRETURN_t am243x_hcd_cancel_transfer(void *hcd_ctx, xUSBH_Transfer_t *transfer)
{
    ...
    return am243x_hcd_status_from_cdn(p_usbssp_obj->stopEndpoint(&usbssp_resources, ep_index));
#else
    return xRETURN_xERR_xUSBH_UNSUPPORTED_OPERATION;
#endif
}
...
static uint32_t am243x_hcd_get_frame_number(void *hcd_ctx)
{
    const xUSBH_AM243x_HCD_Context_t *ctx = (const xUSBH_AM243x_HCD_Context_t *)hcd_ctx;
    ...
    if ((ctx->is_hardware_initialized == true) && (p_usbssp_obj != NULL))
    {
        ...
        {
            ((xUSBH_AM243x_HCD_Context_t *)ctx)->frame_number = frame_number;
        }
    }
#endif
...
void xUSBH_AM243x_HCD_IRQ_Handler(uint8_t port)
{
    ...
    if ((port == xUSBH_AM243X_HCD_USB0_PORT) && (ctx->is_hardware_initialized == true) && (p_usbssp_obj != NULL))
    {
        (void)p_usbssp_obj->isr(&usbssp_resources);
        am243x_hcd_configure_complete_poll(ctx);
        (void)am243x_hcd_poll_port_connection(ctx, true);
    }
#else
    (void)port;
...
const xUSBH_HCD_Ops_t xUSBH_AM243x_HCD_Ops = {
    .init = am243x_hcd_init,
    .deinit = am243x_hcd_deinit,
    .start = am243x_hcd_start,
    .stop = am243x_hcd_stop,
    .enable_interrupts = am243x_hcd_enable_interrupts,
    .disable_interrupts = am243x_hcd_disable_interrupts,
    .port_power = am243x_hcd_port_power,
    .port_reset = am243x_hcd_port_reset,
    .get_port_status = am243x_hcd_get_port_status,
    .submit_transfer = am243x_hcd_submit_transfer,
    .cancel_transfer = am243x_hcd_cancel_transfer,
    .get_frame_number = am243x_hcd_get_frame_number,
};
// EOF /////////////////////////////////////////////////////////////////////////////
